...
class MessageMixin(object):
    ...
    def new_messages(self, messages, saved=False):
        ...
        cls.waiters = []
        if not saved:
            cls.redis_conn.push('chat:messages',messages)
        if cls.redis_conn.llen('chat:messages') > self.cache_size:
...
class MessageNewHandler(BaseHandler, MessageMixin):
    @tornado.web.authenticated
    def post(self):
        MessageMixin.local_cursor = this_cursor = MessageMixin.redis_conn.incr('chat:cursor')
        message = {
            "id": str(this_cursor),
...
class QueryRedis(BaseHandler, MessageMixin):
    def __init__(self):
        cls = MessageMixin
        redis_cursor = cls.redis_conn.get('chat:cursor')
        difference = int(redis_cursor) - int(cls.local_cursor)
        if difference:
            redis_msgs = cls.redis_conn.lrange('chat:messages',-difference,-1)
